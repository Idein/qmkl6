...
static
void naive_sgemv_n(const int m, const int n, const float alpha, const float *a,
        const int lda, const float * const x, const int incx, const float beta,
...
static
void naive_sgemv_t(const int m, const int n, const float alpha, const float *a,
        const int lda, const float * const x, const int incx, const float beta,
        float * const y, const int incy)
{
    for (int i = 0; i < n; ++i, ++a) {
...
static
void naive_sgemv(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE trans,
...
static
int test_sgemv_single(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE trans,
        const int m, const int n, const float alpha, const int lda,
        const int incx, const float beta, const int incy, Generator &gen)
{
    float *a, *x, *y0, *y1;
    std::uniform_real_distribution <float> dist;

    printf("layout = %s, trans = %7s, m = %5d, n = %5d, alpha = %f, lda = %5d, "
            "incx = %d, beta = %f, incy = %d: ",
            (layout == CblasRowMajor) ? "RowMajor" : "ColMajor",
            (trans == CblasNoTrans) ? "NoTrans" : "Trans",
    ...
    printf("err_abs = [%f, %f], err_rel = [%f, %f], %f sec, %f Mflop/s\n",
            ...
            end - start, (2. * xlen * ylen + ylen) / (end - start) * 1e-6);
...
template <class Generator>
static
int test_sgemv_random(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE trans,
        Generator &gen)
{
    ...
    return test_sgemv_single(
            layout, trans, m, n, alpha, lda, incx, beta, incy, gen);
}
...
int main(void)
{
    ...
    for (CBLAS_LAYOUT layout : {CblasRowMajor, CblasColMajor}) {
        for (CBLAS_TRANSPOSE trans : {CblasNoTrans, CblasTrans}) {
            ...
            ret = test_sgemv_single(
                    layout, trans, m, n, 1.f, lda, 1, 1.f, 1, gen);
            if (ret)
                return ret;
        }
    }
