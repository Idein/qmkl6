...
static
void naive_sgemm_rnn(const int M, const int N, const int K, const float alpha,
...
static
void naive_sgemm_rnt(const int M, const int N, const int K, const float alpha,
        const float *A, const int lda, const float *B, const int ldb,
        const float beta, float *C, const int ldc)
{
...
static
void naive_sgemm_rtn(const int M, const int N, const int K, const float alpha,
        const float *A, const int lda, const float *B, const int ldb,
        const float beta, float *C, const int ldc)
{
...
static
void naive_sgemm_rtt(const int M, const int N, const int K, const float alpha,
        const float *A, const int lda, const float *B, const int ldb,
        const float beta, float *C, const int ldc)
{
...
static
void naive_sgemm(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE transa,
        ...
        const int ldb, const float beta, float *c, const int ldc)
{
    ...
    else if (layout == CblasRowMajor && transa == CblasNoTrans && transb == CblasTrans)
        naive_sgemm_rnt(m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
    else if (layout == CblasRowMajor && transa == CblasTrans   && transb == CblasNoTrans)
        naive_sgemm_rtn(m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
    else if (layout == CblasRowMajor && transa == CblasTrans   && transb == CblasTrans)
        naive_sgemm_rtt(m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
    else if (layout == CblasColMajor && transa == CblasNoTrans && transb == CblasNoTrans)
        naive_sgemm_rnn(n, m, k, alpha, b, ldb, a, lda, beta, c, ldc);
    else if (layout == CblasColMajor && transa == CblasNoTrans && transb == CblasTrans)
        naive_sgemm_rtn(n, m, k, alpha, b, ldb, a, lda, beta, c, ldc);
    else if (layout == CblasColMajor && transa == CblasTrans   && transb == CblasNoTrans)
        naive_sgemm_rnt(n, m, k, alpha, b, ldb, a, lda, beta, c, ldc);
    else if (layout == CblasColMajor && transa == CblasTrans   && transb == CblasTrans)
...
static
int test_sgemm_single(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE transa,
        const CBLAS_TRANSPOSE transb, const int m, const int n, const int k,
        const float alpha, const int lda, const int ldb, const float beta,
        const int ldc, Generator &gen)
{
    float *A, *B, *C0, *C1;
    std::uniform_real_distribution <float> dist;

    printf("%c%c%c: m = %5d, n = %5d, k = %5d, alpha = %f, lda = %5d, "
    ...
    if ((layout == CblasRowMajor) == (transa == CblasNoTrans)) {
        if (lda < k) {
            std::cerr << "error: lda must be >= k for row major, no trans type" << std::endl;
            std::exit(EXIT_FAILURE);
        }
        ...
        if (lda < m) {
            std::cerr << "error: lda must be >= m for row major, trans type" << std::endl;
            std::exit(EXIT_FAILURE);
        }
    ...
    if ((layout == CblasRowMajor) == (transb == CblasNoTrans)) {
        if (ldb < n) {
            std::cerr << "error: ldb must be >= n for row major, no trans type" << std::endl;
            std::exit(EXIT_FAILURE);
        }
        ...
        if (ldb < k) {
            std::cerr << "error: ldb must be >= k for row major, trans type" << std::endl;
            std::exit(EXIT_FAILURE);
        }
    ...
    naive_sgemm(layout, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C0, ldc);
    ...
    printf("err_abs = [%f, %f], err_rel = [%f, %f], %f sec, %f Gflop/s\n",
            err_abs_min, err_abs_max, err_rel_min, err_rel_max,
            end - start, (2. * k * n * m + 3. * n * m) / (end - start) * 1e-9);
...
template <class Generator>
static
int test_sgemm_random(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE transa,
...
int main(void)
{
    ...
    for (CBLAS_LAYOUT layout : {CblasRowMajor, CblasColMajor}) {
        ...
        for (CBLAS_TRANSPOSE transa : {CblasNoTrans, CblasTrans}) {
            const int lda = ((layout == CblasRowMajor) == (transa == CblasNoTrans)) ? k : m;
            for (CBLAS_TRANSPOSE transb : {CblasNoTrans, CblasTrans}) {
                const int ldb = ((layout == CblasRowMajor) == (transb == CblasNoTrans)) ? n : k;
                ret = test_sgemm_single(layout, transa, transb, m, n, k, alpha,
                        lda, ldb, beta, ldc, gen);
                if (ret)
                    return ret;
            }
        }
