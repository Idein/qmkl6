...
def qpu_sgemv_t(asm, *, num_qpus, code_offset, align_cond=lambda pos: True):
    ...
    nop(sig=ldunifrf(r3))
    shl(r0, r3, 2, cond='ifa')

    # A_ADDR += 4 * qpu_num - 4 * LDA
    sub(null, r4, IDX0_A_ADDR_ORIG, cond='pushz')
    nop(sig=ldunifrf(r3))
    rotate(broadcast, r0, -IDX0_LDA)
    shl(r2, r1, 2).sub(r3, r3, r5)
    add(r0, r3, r2, cond='ifa')
    ...
    nop(sig=ldunifrf(r3))
    shr(r3, r3, 6 + 4)
    sub(r0, r3, 1, cond='ifa')

    # N = N / num_qpus - 1
    sub(null, r4, IDX0_N_ORIG, cond='pushz')
    nop(sig=ldunifrf(r3))
    shr(r3, r3, num_qpus_shift)
    sub(r0, r3, 1, cond='ifa')
    ...
    with loop as lm:
        ...
        # A_ADDR = A_ADDR_ORIG
        # A_ADDR_ORIG += 4 * LDA * 64 * 16
        rotate(broadcast, r0, -IDX0_A_ADDR_ORIG)
        sub(null, r4, IDX0_A_ADDR, cond='pushz')
        mov(r0, r5, cond='ifa').rotate(broadcast, r0, -IDX0_LDA)
        shl(r5, r5, 6 + 4)
        sub(null, r4, IDX0_A_ADDR_ORIG, cond='pushz')
        ...
        # N = N_ORIG
        rotate(broadcast, r0, -IDX0_N_ORIG)
        ...
        with loop as ln:
            ...
            # r3 = .0
            # r4 = 4 * LDA * eidx but eidx = 0 means eidx = 16
            # r5 = A_ADDR + 4 * qpu_num - 4 * LDA
            # A_ADDR += 4 * num_qpus
            eidx(r4, cond='pushz').rotate(broadcast, r0, -IDX0_LDA)
            add(r4, 8, 8, cond='ifa').mov(r3, r5)
            eidx(r3).umul24(r4, r4, r3)
            rotate(broadcast, r0, -IDX0_A_ADDR)
            sub(null, r3, IDX0_A_ADDR, cond='pushz')
            mov(r3, 1)
            shl(r3, r3, 2 + num_qpus_shift)
            add(r0, r0, r3, cond='ifa').sub(r3, r3, r3)
            ...
            mov(tmua, r4).rotate(broadcast, r0, -IDX0_N, cond='pushz')
            ln.b(cond='na0')
        ...
        rotate(broadcast, r0, -IDX0_M, cond='pushz')
        lm.b(cond='na0')
...
def main():
    ...
    for insn in assemble(qpu_sgemv_t, num_qpus=num_qpus,
