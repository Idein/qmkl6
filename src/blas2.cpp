        ...
        float *y, const int incy)
{
    ...
    if (layout == CblasRowMajor) {
        if (lda < n) {
            fprintf(stderr, "error: lda (%d) must not be smaller than n (%d)"
                    " for row major\n", lda, n);
            XERBLA(1);
        }
    } else {
    ...
    if (use_qpu) {
        ...
        qmkl6.execute_qpu_code(
                (layout == CblasRowMajor) == (trans == CblasNoTrans) ?
                        qmkl6.qpu_sgemv_n_bus : qmkl6.qpu_sgemv_t_bus,
