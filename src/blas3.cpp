...
void cblas_sgemm(const CBLAS_LAYOUT layout, const CBLAS_TRANSPOSE transa,
  ...
  if ((layout == CblasRowMajor) == (transa == CblasNoTrans)) {
    if (lda < k) {
      fprintf(stderr,
              "error: lda (%d) must not be smaller than k (%d)"
  ...
  if ((layout == CblasRowMajor) == (transb == CblasNoTrans)) {
    if (ldb < n) {
      fprintf(stderr,
              "error: ldb (%d) must not be smaller than n (%d)"
              " for row major, no trans type\n",
              ldb, n);
      XERBLA(1);
    }
    ...
    if (ldb < k) {
      fprintf(stderr,
              "error: ldb (%d) must not be smaller than k (%d)"
              " for row major, trans type\n",
              ldb, k);
      XERBLA(1);
    }
  ...
  if (layout == CblasRowMajor) {
    if (ldc < n) {
      fprintf(stderr,
              "error: ldc (%d) must not be smaller than n (%d)"
              " for row major\n",
              ldc, n);
      XERBLA(1);
    }
    ...
    if (ldc < m) {
      fprintf(stderr,
              "error: ldc (%d) must not be smaller than m (%d)"
              " for column major\n",
              ldc, m);
      XERBLA(1);
    }
